...
void test(){
    ...
    MyDB_BufferManagerPtr myMgr = make_shared<MyDB_BufferManager>(1024, 16, "tempFile");
//    MyDB_BufferManagerPtr myMgr = make_shared<MyDB_BufferManager>(200, 16, "tempFile");
    MyDB_PageHandle zero_page = myMgr->getPage(allTables["supplier"], 0);
    MyDB_PageReaderWriter pageReaderWriter(zero_page, true);
    cout << endl;
    cout << "start my own test" << endl;
    cout << "page type : " << pageReaderWriter.getType() << endl;
    string list[2] = {
            "1|Supplier#000000001|sdrGnXCDRcfriBvY0KL,ipCanOTyK t NN1|17|27-918-335-1736|5755.94|requests haggle carefully. accounts sublate finally. carefully ironic pa|",
            "2|Supplier#000000002|TRMhVHz3XiFuhapxucPo1|5|15-679-861-2259|4032.68|furiously stealthy frays thrash alongside of the slyly express deposits. blithely regular req|"
    };
    // store 2 record into the page
    for (const auto &i : list) {
        MyDB_RecordPtr recordPtr = make_shared<MyDB_Record>(allTables["supplier"]->getSchema());
        recordPtr->fromString(i);
        pageReaderWriter.append(recordPtr);
    }
    MyDB_RecordPtr emptyRecPtr = make_shared<MyDB_Record>(allTables["supplier"]->getSchema());
    MyDB_RecordIteratorPtr myDB_pageRecIterator = pageReaderWriter.getIterator(emptyRecPtr);
    while(myDB_pageRecIterator->hasNext()){
        myDB_pageRecIterator->getNext();
        cout << emptyRecPtr << endl;
    }
    // clear the page
    cout << "clear the page reader writer" << endl;
    pageReaderWriter.clear();
    myDB_pageRecIterator = pageReaderWriter.getIterator(emptyRecPtr);
    while(myDB_pageRecIterator->hasNext()){
        myDB_pageRecIterator->getNext();
        cout << emptyRecPtr << endl;
    }
